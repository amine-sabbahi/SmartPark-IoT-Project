...
print ("Program to detect the presence of a car in the parking area, wait 5 seconds before the status changes\nLarge frame size 960x720")
while(cap.isOpened()):   
	spot = 0
	occupied = 0 
    	# Read frame-by-frame    
		...
		for ind, park in enumerate(parking_data):
			...
			# If status is still different than the one saved and counter is open
				...
				if video_cur_pos - parking_buffer[ind] > config['park_sec_to_wait']:
					...
					crsr = cnxn.cursor()
					sql = "UPDATE parking SET status='" + str(status) + "' WHERE no_area=" + str(area)
					crsr.execute(sql)
					cnxn.commit()
					crsr.close()
					cnxn.close()
